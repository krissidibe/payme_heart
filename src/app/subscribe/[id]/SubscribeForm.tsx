...
export default function SubscribeForm() {
  ...
  const contries = [
    {"Capital": "Bamako", "Code": "ML", "Continent": "Africa", "Currency": "FCFA", "Languages": "fr", "Name": "Mali", "Native": "Mali", "Phone": 223},
    {"Capital": "Yamoussoukro", "Code": "CI", "Continent": "Africa", "Currency": "FCFA", "Languages": "fr", "Name": "Côte d'Ivoire", "Native": "Côte d'Ivoire", "Phone": 225},
   {"Capital": "Dakar", "Code": "SN", "Continent": "Africa", "Currency": "FCFA", "Languages": "fr", "Name": "Sénégal", "Native": "Sénégal", "Phone": 221},
   {"Capital": "Ouagadougou", "Code": "BF", "Continent": "Africa", "Currency": "FCFA", "Languages": "fr,ff", "Name": "Burkina Faso", "Native": "Burkina Faso", "Phone": 226},
   {"Capital": "Yaoundé", "Code": "CM", "Continent": "Africa", "Currency": "XAF", "Languages": "en,fr", "Name": "Cameroun", "Native": "Cameroon", "Phone": 237},
   {"Capital": "Conakry", "Code": "GN", "Continent": "Africa", "Currency": "GNF", "Languages": "fr,ff", "Name": "Guinée", "Native": "Guinée", "Phone": 224},
   {"Capital": "Libreville", "Code": "GA", "Continent": "Africa", "Currency": "XAF", "Languages": "fr", "Name": "Gabon", "Native": "Gabon", "Phone": 241},
   {"Capital": "Porto-Novo", "Code": "BJ", "Continent": "Africa", "Currency": "FCFA", "Languages": "fr", "Name": "Benin", "Native": "Bénin", "Phone": 229}
  ]
...
const request = await fetch(
  `${
   BASE_API_URL
  }/api/intouch/malitest?userId=${USER_ID}`,
...
return datas;
   ...
   /*  const request = await fetch(
    ...
    const request = await fetch(
      ...
      }/api/user?userId=${USER_ID}&stripeCustomerId`,
      {
        headers: {
          "Content-type": "application/json",
  ...
  useEffect(() => {
    (async () => {
      ...
      setUserCurrency((x:any) => (x = getCurrency(dataUser?.enterprise?.currency)));
    //  setIsLoading(false)
...
{/* Items */}
<div className="flex flex-col flex-1 w-full gap-4 " style={{gap:15}}>
          <ItemElement
            label="3 mois"
            price="9.890"
            pack="Pack Access"
            ...
            handleClick={()=>{ setCurrentIndex(0)}}
          />
          <ItemElement
         isActive={currentIndex == 1}
         handleClick={()=>{ setCurrentIndex(1)}}
            label="6 mois"
            price="18.950"
              priceIA="950"
            pack="Premium"
...
<Drawer>
      ...
      <div className="flex items-center justify-center mt-5 mb-4 rounded-xl h-14"
        ...
        style={{
                          ...
                          opacity: dataPayment == null ? .3 : 1
        
                        }}>Confirmer</div>
      </DrawerTrigger>
      <DrawerContent className="flex flex-col h-[100%]   overflow-y-scroll  bg-gradient-to-b from-[#2A2A2A] to-[#070707] ">


      {otpModal && <div className="absolute inset-0 z-50 flex items-center px-4 bg-black/50">
      

      <div
 
          className={clsx("p-4 px-5     flex flex-col  pb-[10px] w-full rounded-xl bg-gradient-to-b from-[#191919FF] to-[#222222]  ", )}
...
/>
<p className="mt-1 text-sm text-left opacity-50">Composez le  
{currentOperateur?.country == "Sénégal" ? "#144#391#" : currentOperateur?.country == "Côte d'Ivoire" ?  "#144*82#" : currentOperateur?.country == "Guinée Conakry" ? "*144*4*2*1#" : `*144*4*6*${currentData.amount}*codepin#` } 
 <br />
...
<div
 ...
 onClick={ async () => {  
  ...
  currentData.country = currentData.country == "Guinée" ? "Guinée Conakry" :  currentData.country == "Burkina Faso" ? "Burkina Fasso" :   currentData.country;
...
Confirmer
      ...
      {pendindMoney && <div className="absolute inset-0 z-50 flex items-end bg-black/50">
      

      <div
 
          className={clsx("p-4 px-5  rounded-tl-3xl  rounded-tr-3xl flex flex-col  pb-[10px] bg-gradient-to-b from-[#191919FF] to-[#222222]  ", )}
        ...
        >
         ...
         <LoaderIcon  className="self-center w-10 h-10 animate-spin "  color="#FFFFFFA7" /> 
          </div>
...
Nous vous avons envoyé un SMS avec les instructions de paiement, merci de le vérifier
...
{JSON.stringify(dataPayment)}
          ...
          price={dataPayment?.amount.toString()}
            ...
            creditIA={ typePackage ? dataPayment?.packIA : dataPayment?.month}
...
<div className="p-2 border border-[#ffffff22] flex flex-row mb-7 mt-3  items-center rounded-full w-full">
            ...
            <div
              ...
              className="h-[50px] flex cursor-pointer  bg-[#ffffff1a] w-[50%] rounded-full items-center justify-center"
              style={{
                backgroundColor: typePackage2 ? "#ffffff1a" : "transparent",
              }}
            >
            ...
            onClick={ async ()=> {
              ...
              const plansReq = await fetch(`${process.env.BASE_API_URL}/api/stripe/plans`)
           ...
           await updateUserStripeCustomerId(stripeCustomerId.id) 
...
{/* Pays */}
          ...
          {countriesIntouch.filter(x => x.name == curentCountry?.Name )[0] && 
            

            countriesIntouch.filter(x => x.name == curentCountry?.Name )[0].methods.map((item:any, index:number) => (

              <div
              key={index}
              style={{
                borderColor:item[currentOperateur?.name] ? "#fff" : "transparent",
                borderWidth:item[currentOperateur?.name] ? 1 : 1,
  ...
  if( dataPayment?.type.includes("crédit")) {    currentData.service =  dataPayment?.type.includes("crédit") ? "PaymentIA" : null
 }
            //    console.log(currentData);
             ...
             //   console.log(currentOperateur.name);
              ...
              >
                ...
                height={75}
                alt=""
                style={{ borderColor: "#ffffff", borderWidth:0.5, borderRadius:100  }}
                src={item.image}
              />
              </div>
   ...
   <div className="flex w-full gap-4">
    <DrawerClose asChild >
  <div className="flex items-center justify-center w-full mt-5 mb-4 font-semibold rounded-xl h-14"
...
<div className="flex items-center justify-center w-full mt-5 mb-4 font-semibold rounded-xl h-14"
        onClick={ 
            ...
            if( currentOperateur?.name == undefined && currentData?.number.length < 6){
            ...
            if( (currentOperateur?.country == "Côte d'Ivoire" &&
              currentOperateur?.name == "OrangeMoney") || currentOperateur?.country == "Sénégal" &&
              currentOperateur?.name == "OrangeMoney"  || currentOperateur?.country == "Burkina Faso" &&
              currentOperateur?.name == "OrangeMoney"  || currentOperateur?.country == "Guinée" &&
          ...
          style={{
            backgroundColor: "#9A9768" ,
            opacity: (currentOperateur?.name == undefined || currentData?.number?.length < 6 || currentData?.number?.length == undefined || pendindButton == true) ? 0.3 : 1,
          }}
        >Payer  </div>
   </div>
      ...
      <div
        onClick={ ()=> {
          ...
          const dataPaymentNew: any = {
            ...
            amount: parseInt( typePackage ? price?.replace(".","") +"" : priceIA?.replace(".","")  +""),
        ...
        className={clsx("  flex  bg-[#ffffff05] cursor-pointer  flex-row px-4 py-3 pr-3 items-center   rounded-2xl",isActive ? "border border-white bg-white/10" : "border border-transparent")}
        style={style}
      >
  ...
  { isActive ? <Image
              ...
              width={26}
              height={26}
                src="/images/radioempty.png"
                alt=""
              />
            
            }  
        <div className="flex flex-col justify-center flex-1 pl-3" style={{ gap:0 }}>
       
          <p className="text-[24px] font-semibold">{typePackage ? label : creditIA + " crédits"}</p>
          <p className="text-base leading-5">{typePackage ?  userCurrency == "FCFA" ? price : parseFloat((parseFloat(`${price?.replace(".","")}`)/669.60).toString()).toFixed(2)  : 
         userCurrency == "FCFA" ? priceIA : parseFloat((parseFloat(`${priceIA?.replace(".","")}`)/669.60).toString()).toFixed(2)
        }
...
<Image
              ...
              height={18}
                ...
                alt=""
              />
       <p className="text-base leading-5 ">{typePackage ? creditPack + " crédit IA" : packIA} </p>
          </div>
  ...
  { isActive ? <Image
        ...
        <div className="flex flex-col justify-center flex-1 pl-3" style={{ gap:0 }}>
          ...
          <p className="text-base leading-5">
            ...
            {typePackage ?  userCurrency == "FCFA" ? price : parseFloat((parseFloat(`${price?.replace(".","")}`)/669.60).toString()).toFixed(2)  : 
         userCurrency == "FCFA" ? priceIA : 
         
         parseFloat((parseFloat(`${priceIA}`)/669.60).toString()).toFixed(2)
        }
...
<Image
              ...
              height={18}
                ...
                alt=""
              />
       <p className="text-base leading-5 ">{typePackage ? creditPack + " crédit IA" : pack} </p>
          </div>

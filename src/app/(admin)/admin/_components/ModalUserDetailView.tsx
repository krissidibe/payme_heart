...
import { AlertCircleIcon, EyeIcon, RefreshCwIcon, XIcon } from "lucide-react";
...
import {
  ...
  updateSubscribe,
  updateSubscribeEnd,
} from "../queries-actions/user.action";
...
const ExportAsExcelBtn = ({ data }: { data: any }) => {
  return (
    ...
    >
      {(props) => (
        <button
          className="items-center cursor-pointer justify-center bg-white text-black text-sm font-semibold flex rounded-md w-[100px]"
          {...props}
        >
          Exporter
        </button>
...
function ModalUserDetailView({
  ...
  const [startAt, setStartAt] = useState<string>(
    `${new Date(Date.now()).getFullYear()}-${(
      parseInt(new Date(Date.now()).getMonth().toString()) + 1
    )
      .toString()
  ...
  function calculeTotal(data: any): number {
    ...
    let total = 0;
    data.map((item: any) => {
      const stripeValue = item.type == "Stripe" ? 669.6 : 1;
                                  ...
                                  return {
                                    ...
                                    Abonnée:
                                      user.subscribe?.payment?.month != 0
                                        ? `${user.subscribe?.payment?.month} mois`
                                        : "Non abonnée",
                                    projet: "",
                                  };
                                }
                                return {
                                  "Date d'inscription": dayjs(
                                    `${user.createdAt}`
                                  )
                                    .format("DD/MM/YYYY")
                                    .replace("T", " "),
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        {dataPayment != null && (
                          <p>
                            ...
                            Total :{" "}
                            {parseFloat(
                              calculeTotal(dataPayment).toString()
                            ).toFixed(2)}
                          </p>
                        )}
                        {dataPayment != null && (
                      ...
                      {dataUser?.map((user: any, index: number) => (
                        <div
                          className={clsx(
                            ...
                            user.subscribe?.payment?.month == 3
                              ? "bg-green-300/30"
                              : "",
                            user.subscribe?.payment?.month == 6
                              ? "bg-amber-300/30"
                              : "",
                            user.subscribe?.payment?.month == 12
                              ? "bg-blue-300/30"
                              : ""
                          )}
                        >
                            ...
                            <Dialog>
                              ...
                              <DialogContent className="sm:max-w-[425px]">
                                <form
                                  onSubmit={async (e) => {
                                    e.preventDefault();
                                    const dataNew = await updateSubscribe(
                                      user.subscribe.id
                                    );

                                    if (dataNew.id != null) {
                                      toast(
                                        "Modification efectuer avec succes ",
                                        {
                            ...
                            <Dialog>
                              ...
                              <DialogContent className="sm:max-w-[425px]">
                                <form
                                  onSubmit={async (e) => {
                                    e.preventDefault();
                                    const dataNew = await updateSubscribeEnd(
                                      user.subscribe.id
                                    );
                            ...
                            <Dialog>
                              ...
                              <DialogContent className="sm:max-w-[425px]">
                                <form
                                  onSubmit={async (e) => {
                                    e.preventDefault();
                                    const dataNew = await updateCreditIA(
                                      user.id
                                    );
                      ...
                      {dataPayment?.map((payment: any) => (
                        <div className="flex w-full p-6 border-b shadow-sm">
                          ...
                          <div className="flex flex-col w-1/3">
                            ...
                            <p>
                              Secteur : {payment.user?.enterprise?.activity}
                            </p>
                          </div>

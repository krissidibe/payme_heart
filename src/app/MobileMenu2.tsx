   ...
   <img src={"/images/logo-payme-complet.png"} width={140} height={50} />
    ...
    <ToggleButton isOpen={isOpen} handleClick={()=>{
        ...
        <div className={`${isOpen ? "scale-100 right-0 transition-all duration-300" :"scale-0 rotate-0 transition-all duration-300"} absolute z-50 flex flex-col h-[200px] w-full gap-2 px-10 py-4 rounded-md right-0   top-10`}>
          <Link href={`${process.env.BASE_API_URL}/`} onClick={()=> setIsOpen(x=>!x)}  className="cursor-pointer hover:text-white">Acceuil   </Link>
          <Link href={`${process.env.BASE_API_URL}/`} onClick={()=> setIsOpen(x=>!x)}  className="cursor-pointer hover:text-white">Caractéristiques</Link>
          <Link href={`${process.env.BASE_API_URL}/`} onClick={()=> setIsOpen(x=>!x)}  className="cursor-pointer hover:text-white">Cible</Link>
          <Link href={`${process.env.BASE_API_URL}/`} onClick={()=> setIsOpen(x=>!x)}  className="cursor-pointer hover:text-white">FAQ</Link>
          <Link href={`${process.env.BASE_API_URL}/`} onClick={()=> setIsOpen(x=>!x)}  className="cursor-pointer hover:text-white">Télécharger</Link>

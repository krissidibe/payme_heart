...
const monthValue = parseInt(dataPayment.month.toString()!)
  ...
  const reference = `${dataPayment.service  == "PaymentIA" ? "IA-" : ""}${lettersPart}-${numbersPart}-${date}`
  ...
  const callback = `https://paymefinance.com/api/paymentpush?userId=${searchParams.get(
    ...
    dataPayment.amount
  }&type=${typePayment}&reference=${reference}&service=${dataPayment.service}`;

  const callbackTest = `${process.env.BASE_URL}/api/paymentpush?userId=${searchParams.get(
    "userId"
  )!}&month=${dataPayment.month}&amount=${
    dataPayment.amount
  }&type=${typePayment}&reference=${reference}&service=${dataPayment.service}`;

    ...
    const totalDiscount = parseFloat(
      (
        (parseFloat(totalHtCalcule) *
          parseFloat(
            dataNew.project.discount == "" ? 0 : dataNew.project.discount.replace(",",".")
          )) /
        100
      ).toString()
    );
   ...
   */
    ...
    const totalTva = parseFloat(
      (
        ((parseFloat(totalHtCalcule) - parseFloat(totalDiscount.toString())) *
          parseFloat(dataNew.project.tva == "" ? 0 : dataNew.project.tva.replace(",","."))) /
        100
      ).toString()
    );
    ...
    const totalAccount = parseFloat(
      (
        (total *
          parseFloat(
            dataNew.project.modalite == "" ? 0 : dataNew.project.modalite.replace(",",".")
          )) /
        100
      ).toString()
    );
